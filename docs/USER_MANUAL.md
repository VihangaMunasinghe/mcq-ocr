...
3. Sidebar → **Settings** → **Faculties** tab → **Add Faculty** → enter the faculty name → **Create Faculty**.
4. Repeat for every faculty.
5. Create the first Faculty Admin for each faculty: follow [Admin adds a user](#a-admin-adds-a-user) below, then promote the new account to `Faculty Admin` per [Verify, promote, or delete a user](#-verify-promote-or-delete-a-user).
...
Sidebar → **Marking Jobs** → **New Marking Job**. Three sequential steps.
...
- **Pause** — visible only while `Processing`.
- **Delete** — confirm modal.
