...
const Cluster_based_viewer: React.FC<ClusterBasedViewerProps> = ({
  ...
  const handleMouseUp = () => {
    if (!isDragging || !selectedBubble || !mousePos) return;
    ...
    if (!position) {
      //  Outside valid column
      showToast("The bubble must be placed within a valid column's boundaries", "error");
      newBubbles[selectedBubble.colIndex][selectedBubble.rowIndex].push(selectedBubble.originalCoord);
    } else {
      const isSameRow =
        position.col === selectedBubble.colIndex &&
        position.row === selectedBubble.rowIndex;

      //  Case 1: Same row -> just reinsert without validation
      if (isSameRow) {
        newBubbles[position.col][position.row].push({
          x: Math.round(mousePos.x),
          y: Math.round(mousePos.y),
        });
        newBubbles[position.col][position.row].sort((a, b) => a.x - b.x);
      }
      //  Case 2: New row within valid column
      ...
      //  Case 3: Move to another existing row
      else {
        ...
        if (rowLength < maxOptions) {
          ...
          // return bubble to original place
          newBubbles[selectedBubble.colIndex][selectedBubble.rowIndex].push(
    ...
    try {
      ...
      formData.append(
        ...
        JSON.stringify({
          ...
          bubbles: updatedNormalized // ✅ use normalized version
        })
      );
  ...
  const handleMouseDown = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    if (selected !== null) {
      ...
      const newBubbles = bubbles.map((col, i) => {
        if (i !== selected!.colIndex) return col;
        return col.map((row, j) => {
          if (j !== selected!.rowIndex) return row;
          return row.filter((_, k) => k !== selected!.bubbleIndex);
        });
      });
  ...
  }, [templateImage, configData, scale, bubbles, isDragging, mousePos, hoveredBubble]);
