...
def get_clustering(img_path,Num_of_columns,Num_of_rows_per_column,Num_of_options_per_question,want_intermediate_results=False):
    
    num_of_options_per_question= Num_of_options_per_question
    num_of_rows_per_column=Num_of_rows_per_column
    ...
    fixed_width = 1200
    fixed_height = 1600
    resized_img=cv2.resize(warped_img,(fixed_width,fixed_height))
    imGray = cv2.cvtColor(resized_img,cv2.COLOR_BGR2GRAY)

    template_width = imGray.shape[1]

    min_thickness = 5  # Minimum thickness in pixels for a thick line (adjust this)
    ...
    #cv2.waitKey(0)
    #cv2.destroyAllWindows()
    return  bubble_data,warped_img,result_img
